...
public class BrandController {
     ...
     */
    @PostMapping("/test")
    public Brand crate(@Valid @RequestBody Brand brand, BindingResult errors){
        if (errors.hasErrors()){
            errors.getAllErrors().stream().forEach(error-> System.out.println(error.getDefaultMessage())); //数据校验未通过,返回具体信息,可在实体类的字段上指定message信息
        }
        return null;
    }
}
